...
pub async fn get_route_origin_destination(configuration: &configuration::Configuration, params: GetRouteOriginDestinationParams) -> Result<ResponseContent<GetRouteOriginDestinationSuccess>, Error<GetRouteOriginDestinationError>> {
    ...
    // let local_var_uri_str = format!("{}/route/{origin}/{destination}/", configuration.base_path, destination=destination, origin=origin);
    // let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());

    // if let Some(ref local_var_str) = avoid {
    //     local_var_req_builder = local_var_req_builder.query(&[("avoid", &local_var_str.into_iter().map(|p| p.to_string()).collect::<Vec<String>>().join(",").to_string())]);
    // }
    // if let Some(ref local_var_str) = connections {
    //     local_var_req_builder = local_var_req_builder.query(&[("connections", &local_var_str.into_iter().map(|p| p.to_string()).collect::<Vec<String>>().join(",").to_string())]);
    // }
    ...
    //     let local_var_entity: Option<GetRouteOriginDestinationSuccess> = serde_json::from_str(&local_var_content).ok();
